// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'hr_controller.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
  'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#adding-getters-and-methods-to-our-models',
);

/// @nodoc
mixin _$HRState {
  List<TimeEntry> get todayEntries => throw _privateConstructorUsedError;
  List<ApprovalRequest> get pendingRequests =>
      throw _privateConstructorUsedError;
  bool get isLoading => throw _privateConstructorUsedError;
  String? get error => throw _privateConstructorUsedError;

  /// Create a copy of HRState
  /// with the given fields replaced by the non-null parameter values.
  @JsonKey(includeFromJson: false, includeToJson: false)
  $HRStateCopyWith<HRState> get copyWith => throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $HRStateCopyWith<$Res> {
  factory $HRStateCopyWith(HRState value, $Res Function(HRState) then) =
      _$HRStateCopyWithImpl<$Res, HRState>;
  @useResult
  $Res call({
    List<TimeEntry> todayEntries,
    List<ApprovalRequest> pendingRequests,
    bool isLoading,
    String? error,
  });
}

/// @nodoc
class _$HRStateCopyWithImpl<$Res, $Val extends HRState>
    implements $HRStateCopyWith<$Res> {
  _$HRStateCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  /// Create a copy of HRState
  /// with the given fields replaced by the non-null parameter values.
  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? todayEntries = null,
    Object? pendingRequests = null,
    Object? isLoading = null,
    Object? error = freezed,
  }) {
    return _then(
      _value.copyWith(
            todayEntries: null == todayEntries
                ? _value.todayEntries
                : todayEntries // ignore: cast_nullable_to_non_nullable
                      as List<TimeEntry>,
            pendingRequests: null == pendingRequests
                ? _value.pendingRequests
                : pendingRequests // ignore: cast_nullable_to_non_nullable
                      as List<ApprovalRequest>,
            isLoading: null == isLoading
                ? _value.isLoading
                : isLoading // ignore: cast_nullable_to_non_nullable
                      as bool,
            error: freezed == error
                ? _value.error
                : error // ignore: cast_nullable_to_non_nullable
                      as String?,
          )
          as $Val,
    );
  }
}

/// @nodoc
abstract class _$$HRStateImplCopyWith<$Res> implements $HRStateCopyWith<$Res> {
  factory _$$HRStateImplCopyWith(
    _$HRStateImpl value,
    $Res Function(_$HRStateImpl) then,
  ) = __$$HRStateImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call({
    List<TimeEntry> todayEntries,
    List<ApprovalRequest> pendingRequests,
    bool isLoading,
    String? error,
  });
}

/// @nodoc
class __$$HRStateImplCopyWithImpl<$Res>
    extends _$HRStateCopyWithImpl<$Res, _$HRStateImpl>
    implements _$$HRStateImplCopyWith<$Res> {
  __$$HRStateImplCopyWithImpl(
    _$HRStateImpl _value,
    $Res Function(_$HRStateImpl) _then,
  ) : super(_value, _then);

  /// Create a copy of HRState
  /// with the given fields replaced by the non-null parameter values.
  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? todayEntries = null,
    Object? pendingRequests = null,
    Object? isLoading = null,
    Object? error = freezed,
  }) {
    return _then(
      _$HRStateImpl(
        todayEntries: null == todayEntries
            ? _value._todayEntries
            : todayEntries // ignore: cast_nullable_to_non_nullable
                  as List<TimeEntry>,
        pendingRequests: null == pendingRequests
            ? _value._pendingRequests
            : pendingRequests // ignore: cast_nullable_to_non_nullable
                  as List<ApprovalRequest>,
        isLoading: null == isLoading
            ? _value.isLoading
            : isLoading // ignore: cast_nullable_to_non_nullable
                  as bool,
        error: freezed == error
            ? _value.error
            : error // ignore: cast_nullable_to_non_nullable
                  as String?,
      ),
    );
  }
}

/// @nodoc

class _$HRStateImpl implements _HRState {
  const _$HRStateImpl({
    final List<TimeEntry> todayEntries = const [],
    final List<ApprovalRequest> pendingRequests = const [],
    this.isLoading = false,
    this.error,
  }) : _todayEntries = todayEntries,
       _pendingRequests = pendingRequests;

  final List<TimeEntry> _todayEntries;
  @override
  @JsonKey()
  List<TimeEntry> get todayEntries {
    if (_todayEntries is EqualUnmodifiableListView) return _todayEntries;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_todayEntries);
  }

  final List<ApprovalRequest> _pendingRequests;
  @override
  @JsonKey()
  List<ApprovalRequest> get pendingRequests {
    if (_pendingRequests is EqualUnmodifiableListView) return _pendingRequests;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_pendingRequests);
  }

  @override
  @JsonKey()
  final bool isLoading;
  @override
  final String? error;

  @override
  String toString() {
    return 'HRState(todayEntries: $todayEntries, pendingRequests: $pendingRequests, isLoading: $isLoading, error: $error)';
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$HRStateImpl &&
            const DeepCollectionEquality().equals(
              other._todayEntries,
              _todayEntries,
            ) &&
            const DeepCollectionEquality().equals(
              other._pendingRequests,
              _pendingRequests,
            ) &&
            (identical(other.isLoading, isLoading) ||
                other.isLoading == isLoading) &&
            (identical(other.error, error) || other.error == error));
  }

  @override
  int get hashCode => Object.hash(
    runtimeType,
    const DeepCollectionEquality().hash(_todayEntries),
    const DeepCollectionEquality().hash(_pendingRequests),
    isLoading,
    error,
  );

  /// Create a copy of HRState
  /// with the given fields replaced by the non-null parameter values.
  @JsonKey(includeFromJson: false, includeToJson: false)
  @override
  @pragma('vm:prefer-inline')
  _$$HRStateImplCopyWith<_$HRStateImpl> get copyWith =>
      __$$HRStateImplCopyWithImpl<_$HRStateImpl>(this, _$identity);
}

abstract class _HRState implements HRState {
  const factory _HRState({
    final List<TimeEntry> todayEntries,
    final List<ApprovalRequest> pendingRequests,
    final bool isLoading,
    final String? error,
  }) = _$HRStateImpl;

  @override
  List<TimeEntry> get todayEntries;
  @override
  List<ApprovalRequest> get pendingRequests;
  @override
  bool get isLoading;
  @override
  String? get error;

  /// Create a copy of HRState
  /// with the given fields replaced by the non-null parameter values.
  @override
  @JsonKey(includeFromJson: false, includeToJson: false)
  _$$HRStateImplCopyWith<_$HRStateImpl> get copyWith =>
      throw _privateConstructorUsedError;
}
